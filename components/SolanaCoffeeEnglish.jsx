...
function AdminPanel({ campaigns, onUpdateCampaigns, onClose, darkMode, publicKey, signMessage }) {
  ...
  const handleApprove = async (campaignId) => {
    try {
      console.log('[ADMIN-APPROVE] Approving campaign:', campaignId);
      
      const response = await fetch('/api/approve-campaign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      if (response.ok) {
        ...
        alert(`Error: ${error.error || 'Failed to approve campaign'}`);
      }
    } catch (error) {
  ...
  const handleSaveEdit = async () => {
    try {
      console.log('[ADMIN-EDIT] Saving campaign edits:', editingCampaign);
      
      const response = await fetch('/api/update-campaign-socials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          campaignId: editingCampaign.id || editingCampaign.campaign_id,
          twitter: editingCampaign.twitter,
          telegram: editingCampaign.telegram,
          website: editingCampaign.website,
          walletAddress: publicKey.toString()
        })
      });
      ...
      console.error('[ADMIN-EDIT] Exception:', error);
...
// CreateCampaign - Version Debug Ultra Simple
// Test d'affichage avant le formulaire complet

function CreateCampaign({ onClose, onCreate, darkMode }) {
  console.log('[CREATE-CAMPAIGN] Component rendered!');
  console.log('[CREATE-CAMPAIGN] darkMode:', darkMode);
  ...
  // AFFICHAGE FORCÉ AVEC Z-INDEX ULTRA-HAUT
  return (
    <div 
      style={{
        position: 'fixed',
      ...
      onClick={(e) => {
        console.log('[CREATE-CAMPAIGN] Overlay clicked');
    ...
    >
      ...
      >
        ...
        <button 
          onClick={() => {
            console.log('[CREATE-CAMPAIGN] Close button clicked');
            onClose();
          }}
          type="button"
          style={{
            ...
            borderRadius: '50%',
            border: 'none',
            backgroundColor: darkMode ? '#334155' : '#F5F1ED',
            color: darkMode ? '#cbd5e1' : '#666',
            fontSize: '24px',
            cursor: 'pointer',
            display: 'flex',
        ...
        <form onSubmit={handleSubmit} style={{ padding: '32px' }}>
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '16px',
              backgroundColor: publicKey ? '#7c3aed' : '#9ca3af',
              color: 'white',
              border: 'none',
              borderRadius: '12px',
              fontSize: '16px',
